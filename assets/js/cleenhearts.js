...
const preloader = document.querySelector('#preloader-comp');
const loaderBtn = document.querySelector('.loader-btn');

// Function to hide the preloader
const hidePreloader = () => {
    preloader.style.display = "none";
    sessionStorage.setItem('preloaderShown', 'true'); // Mark the preloader as shown
};
...
window.addEventListener('load', () => {
    ...
    if (!preloaderShown) {
        preloader.style.display = "grid" ; // Show the preloader if it hasn't been shown before
        setTimeout(hidePreloader, 20000) ; // Hide after 3 seconds or when an event occurs
    } else {
        preloader.style.display = "none" ; // Ensure the preloader remains hidden on subsequent visits
    }
});

// Hide preloader when button is clicked (if applicable)
loaderBtn.addEventListener("click", hidePreloader);


  // Grab the audio element
  const audio = document.getElementById('background-music');

  // Mute the audio initially to avoid autoplay restrictions
  audio.muted = true;
  ...
  window.addEventListener('load', function() {
    setTimeout(() => {
      ...
      audio.play().then(() => {
        console.log('Audio started playing after 3 seconds.');
      }).catch((error) => {
        console.log('Autoplay was blocked due to browser restrictions:', error);
      });
